...
ZEND_API void zend_objects_destroy_object(zend_object *object)
{
	...
	if (destructor) {
		...
		if (destructor->common.fn_flags & (ZEND_ACC_PRIVATE|ZEND_ACC_PROTECTED|ZEND_ACC_MODULE_INTERNAL)) {
			if (EG(current_execute_data)) {
				zend_class_entry *scope = zend_get_executed_scope();
				/* Ensure that if we're calling a non-public function, we're allowed to do so. */
