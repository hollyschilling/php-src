...
int zend_jit_op_array(zend_op_array *op_array, zend_script *script)
{
	...
	if (UNEXPECTED((op_array->generic_params != NULL)
	 ...
	 && JIT_G(trigger) != ZEND_JIT_ON_HOT_TRACE) {
		 ...
		 * clone -- class-level clones and generic-METHOD clones alike --
		 * bypassing the clones' substituted arg_info (a check compiled
		 * from the template rejects everything: "must be of type T"). Keep
		 * these bodies interpreted; hot-trace mode compiles per trace with
		 * runtime function guards and remains enabled. */
		ZEND_SET_FUNC_INFO(op_array, NULL);
		return SUCCESS;
	}
